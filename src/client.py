...
def start_client(client_id, seed, dataset, global_model=None, algorithm='fedavg', global_control=None, model_name='lenet5'):
    ...
    if algorithm.lower() == 'fedewma':
        ...
        logger.info(f"Client {client_id} sending full delta weights")
    else:
        delta_weights = local_model.state_dict()
    ...
    logger.info(f"Sent {data_size_bytes} bytes to server (weights size: {weights_size} bytes, client data size: {data_size}, sparse: {False})")
