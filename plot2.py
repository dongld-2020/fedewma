...
def plot_metrics(csv_files):
    ...
    for algorithm, df in all_data:
        if algorithm == 'fedewma':            
            ax1.plot(df['Round'], df['Loss'], label=f'{algorithm.capitalize()} Loss', linestyle='-', linewidth=3)
        else:
    ...
    for algorithm, df in all_data:
        if algorithm == 'fedewma':
            ax2.plot(df['Round'], df['Accuracy'], label=f'{algorithm.capitalize()} Accuracy', linestyle='-', linewidth=3)
        else:
